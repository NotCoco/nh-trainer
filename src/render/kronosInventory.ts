...
import type { KronosSelectedSpell } from "./kronosSceneObjects";
...
export const KRONOS_INVENTORY_USE_OPCODE = 38;
export const KRONOS_INVENTORY_USE_SELECTED_OPCODE = 31;
export const KRONOS_INVENTORY_SPELL_SELECTED_OPCODE = 32;
export const KRONOS_INVENTORY_EXAMINE_OPCODE = 1005;
export const KRONOS_INVENTORY_WIDGET_ID = 9764864;

export const initialKronosInventorySlots: readonly RuntimeInventorySlot[] = [...gearSwitchSlots, ...supplySlots].map(
  (itemId) => ({
    itemId,
    quantity: 1
  })
...
export function normalizeKronosInventorySlots(
  slots: readonly (RuntimeInventorySlot | null)[] | undefined
): readonly (RuntimeInventorySlot | null)[] {
  const source = slots && slots.length > 0 ? slots : initialKronosInventorySlots;
  return Array.from({ length: inventorySlotCount }, (_, index) => source[index] ?? null);
}

export interface KronosInventoryQuantityText {
  readonly text: string;
  readonly color: string;
}

export interface KronosInventoryItemDefinition {
  readonly id: number;
...
export interface KronosInventoryEquipmentDefinition {
  readonly id: number;
  readonly name: string;
  readonly equipSlot: number | null;
  readonly weaponType: string | null;
  readonly twoHanded: boolean;
  readonly specialAttack: KronosInventorySpecialAttackDefinition | null;
}

export interface KronosInventorySpecialAttackDefinition {
  readonly drainPercent: number;
  readonly source: string;
}

export type KronosInventoryItemDefinitionStore = ReadonlyMap<number, KronosInventoryItemDefinition>;
export type KronosInventoryEquipmentDefinitionStore = ReadonlyMap<number, KronosInventoryEquipmentDefinition>;

export interface KronosInventorySelectedItem {
...
export type KronosInventoryContextMenuEntry = KronosMenuEntry & {
...
export interface KronosInventoryActionMutation {
...
export interface KronosInventoryDragMutation {
...
export function kronosInventoryQuantityText(
  quantity: number,
  itemDefinition: KronosInventoryItemDefinition | undefined,
  quantityMode = 2
): KronosInventoryQuantityText | null {
...
export function createKronosInventoryItemDefinitionStore(source: unknown): KronosInventoryItemDefinitionStore {
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    return new Map();
  }

  const entries: Array<readonly [number, KronosInventoryItemDefinition]> = [];
...
export function createKronosInventoryEquipmentDefinitionStore(source: unknown): KronosInventoryEquipmentDefinitionStore {
  if (!Array.isArray(source)) {
    return new Map();
  }

  const entries: Array<readonly [number, KronosInventoryEquipmentDefinition]> = [];
...
function normalizeInventorySpecialAttack(source: unknown): KronosInventorySpecialAttackDefinition | null {
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    return null;
  }
...
export function buildKronosInventoryContextEntries({
  slot,
  slotIndex,
  widgetId,
  itemDefinition,
...
}): readonly KronosInventoryContextMenuEntry[] {
  const normalizedSlotIndex = Math.max(0, Math.trunc(slotIndex));
  const normalizedWidgetId = Number.isInteger(widgetId) ? Math.trunc(widgetId) : KRONOS_INVENTORY_WIDGET_ID;
  ...
  const entries: KronosInventoryContextMenuEntry[] = [];
    ...
    return [
      {
        ...
        opcode: KRONOS_INVENTORY_USE_SELECTED_OPCODE,
      ...
      return [
        {
          ...
          opcode: KRONOS_INVENTORY_SPELL_SELECTED_OPCODE,
          identifier: slot.itemId,
          argument1: normalizedSlotIndex,
          argument2: normalizedWidgetId,
  ...
  entries.push({
    actionText: "Use",
    targetText,
    opcode: KRONOS_INVENTORY_USE_OPCODE,
    identifier: slot.itemId,
    argument1: normalizedSlotIndex,
    argument2: normalizedWidgetId,
  ...
  entries.push({
    actionText: "Examine",
    targetText,
    opcode: KRONOS_INVENTORY_EXAMINE_OPCODE,
    identifier: slot.itemId,
    argument1: normalizedSlotIndex,
    argument2: normalizedWidgetId,
...
export function mutateKronosInventorySlotsForAction(
  slots: readonly (RuntimeInventorySlot | null)[],
  entry: KronosInventoryContextMenuEntry
): { readonly slots: readonly (RuntimeInventorySlot | null)[]; readonly mutation: KronosInventoryActionMutation | null } {
  ...
  if (action === "eat") {
    return replaceKronosInventorySlot(slots, entry.slotIndex, null, "eat-remove", current.itemId);
  }
  if (action === "drop") {
    return replaceKronosInventorySlot(slots, entry.slotIndex, null, "drop-remove", current.itemId);
  }
  if (action === "empty") {
    return replaceKronosInventorySlot(slots, entry.slotIndex, { itemId: emptyVialItemId, quantity: 1 }, "empty-vial", current.itemId);
  }
  if (action === "drink") {
    ...
    return replaceKronosInventorySlot(slots, entry.slotIndex, { itemId: nextItemId, quantity: 1 }, "drink-dose", current.itemId);
  }

  return { slots, mutation: null };
}

export function replaceKronosInventorySlot(
  slots: readonly (RuntimeInventorySlot | null)[],
  slotIndex: number,
  nextSlot: RuntimeInventorySlot | null,
  kind: KronosInventoryActionMutation["kind"],
  previousItemId: number
): { readonly slots: readonly (RuntimeInventorySlot | null)[]; readonly mutation: KronosInventoryActionMutation } {
  const nextSlots = [...normalizeKronosInventorySlots(slots)];
...
export function reorderKronosInventorySlotsForDrag(
  slots: readonly (RuntimeInventorySlot | null)[],
  sourceSlotIndex: number,
  destinationSlotIndex: number
): { readonly slots: readonly (RuntimeInventorySlot | null)[]; readonly mutation: KronosInventoryDragMutation | null } {
  ...
  if (
    ...
    return { slots: normalizeKronosInventorySlots(slots), mutation: null };
  }
...
function inventoryActionEntry(
  ...
  itemName: string,
  targetText: string
): KronosInventoryContextMenuEntry | null {
