export interface NhSize {
  readonly width: number;
  readonly height: number;
}

export interface NhRect extends NhSize {
  readonly x: number;
  readonly y: number;
}

export interface NhInterfaceWidget {
...
export interface NhInterfaceGroup {
  readonly groupId: number;
  readonly widgets: readonly NhInterfaceWidget[];
}

export interface NhClientWidgetDefinitions {
  readonly source: string;
  readonly fixedCanvas: NhSize;
  readonly groups: readonly NhInterfaceGroup[];
}

export interface NhSpellbookDefinitions {
  readonly source: string;
  readonly spellbooks: readonly NhSpellbookDefinition[];
}

export type NhSpellbookId = "standard" | "ancient" | "lunar" | "arceuus";

export interface NhSpellbookDefinition {
  readonly id: NhSpellbookId;
  ...
  readonly spells: readonly NhSpellbookDefinitionSpell[];
}

export interface NhSpellbookDefinitionSpell {
  readonly id: string;
...
export interface NhResolvedWidget {
  readonly widget: NhInterfaceWidget;
...
export interface NhInventoryGridLayout {
  ...
  readonly containerRect: NhRect;
  readonly rect: NhRect;
  readonly columns: number;
  readonly rows: number;
  readonly slot: NhSize;
  readonly step: NhSize;
  readonly padding: NhSize;
}

export type NhFixedOrbId = "hp" | "prayer" | "run" | "spec";

export interface NhFixedOrbLayout {
  readonly id: NhFixedOrbId;
  readonly groupId: number;
  readonly rect: NhRect;
  readonly actionWidgetId: number | null;
  readonly actionChildId: number | null;
  readonly actionRect: NhRect | null;
  readonly actions: readonly string[];
  readonly frameSpriteId: number;
  readonly frameRect: NhRect;
  readonly fillerSpriteId: number;
  readonly activeFillerSpriteId: number | null;
  readonly fillRect: NhRect;
  readonly emptySpriteId: number;
  readonly emptyRect: NhRect;
  readonly iconSpriteId: number;
  readonly iconRect: NhRect;
  readonly valueTextRect: NhRect | null;
  readonly valueText: NhFixedOrbValueText | null;
}

export interface NhXpDropOrbLayout {
  readonly groupId: number;
  readonly widgetId: number;
  readonly childId: number;
  readonly rect: NhRect;
...
export interface NhFixedOrbValueText {
  readonly rect: NhRect;
  readonly fontId?: number;
  readonly lineHeight?: number;
  readonly xTextAlignment?: number;
...
export interface NhMountedInterfaceLayout {
  readonly groupId: number;
  readonly rect: NhRect;
  readonly widgets: readonly NhResolvedWidget[];
}

export type NhEquipmentSlotId =
  | "head"
  | "cape"
  | "amulet"
  | "weapon"
...
export type NhEquipmentUtilityButtonId = "stats" | "prices" | "items-kept-on-death" | "call-follower";

export interface NhEquipmentUtilityButtonLayout {
  ...
  readonly spriteRect: NhRect | null;
}

export interface NhEquipmentPanelLayout {
  readonly groupId: number;
  readonly rect: NhRect;
  readonly slots: readonly NhEquipmentSlotLayout[];
  readonly utilityButtons: readonly NhEquipmentUtilityButtonLayout[];
}

export interface NhCombatTextLayout {
  readonly widgetId: number;
  readonly childId: number;
  readonly rect: NhRect;
  readonly fontId?: number;
...
export interface NhCombatStyleSlotLayout {
  readonly index: number;
  readonly actionWidgetId: number;
  readonly actionChildId: number;
  readonly actionRect: NhRect;
  ...
  readonly iconRect: NhRect | null;
  readonly text: NhCombatTextLayout;
  readonly actions: readonly string[];
}

export interface NhCombatSpecialBarLayout {
  readonly containerWidgetId: number;
  readonly containerChildId: number;
  readonly containerRect: NhRect;
  readonly actionWidgetId: number;
...
export interface NhCombatAutoRetaliateLayout {
  readonly widgetId: number;
  readonly childId: number;
  readonly rect: NhRect;
  readonly actionText: string;
...
export interface NhCombatAutocastControlLayout {
  ...
  readonly defensiveIconRect: NhRect | null;
  readonly label: NhCombatTextLayout | null;
}

export interface NhCombatPanelLayout {
  readonly groupId: number;
  readonly rect: NhRect;
  readonly weaponName: NhCombatTextLayout | null;
  readonly combatLevel: NhCombatTextLayout | null;
  readonly styleSlots: readonly NhCombatStyleSlotLayout[];
  readonly autocastControls: readonly NhCombatAutocastControlLayout[];
  readonly autoRetaliate: NhCombatAutoRetaliateLayout | null;
  readonly specialBar: NhCombatSpecialBarLayout | null;
}

export type NhPrayerId =
  | "thick-skin"
  | "burst-of-strength"
  | "clarity-of-thought"
  | "sharp-eye"
...
export interface NhPrayerSlotLayout {
...
export interface NhPrayerPanelLayout {
  readonly groupId: number;
  readonly rect: NhRect;
  readonly containerWidgetId: number;
  readonly containerChildId: number;
  ...
  readonly slot: NhSize;
  readonly step: NhSize;
  readonly slots: readonly NhPrayerSlotLayout[];
}

export interface NhSpellbookSpellLayout {
  readonly id: string;
  readonly label: string;
  readonly itemId: number;
  readonly spriteId: number;
...
export type NhSpellbookLayoutMode = "disable-filtering-fixed";

export interface NhSpellbookPanelLayout {
  readonly id: NhSpellbookId;
  readonly enumId: number;
  ...
  readonly layoutMode: NhSpellbookLayoutMode;
  readonly groupId: number;
  readonly rect: NhRect;
  readonly parentWidgetId: number;
  readonly parentChildId: number;
  readonly parentRect: NhRect;
  readonly boundsWidgetId: number;
  readonly boundsChildId: number;
  readonly spellAreaRect: NhRect;
  readonly columns: number;
  readonly rows: number;
  readonly slot: NhSize;
  readonly gap: NhSize;
  readonly step: NhSize;
  readonly spells: readonly NhSpellbookSpellLayout[];
}

export type NhStatsSkillId =
  | "attack"
  | "strength"
  | "defence"
  | "ranged"
...
export interface NhStatsSkillSlotLayout {
  readonly id: NhStatsSkillId;
  readonly label: string;
...
export interface NhStatsTotalLevelLayout {
  readonly containerWidgetId: number;
  readonly containerChildId: number;
  readonly containerRect: NhRect;
...
export interface NhStatsPanelLayout {
  readonly groupId: number;
  readonly rect: NhRect;
  readonly columns: number;
  readonly slot: NhSize;
  readonly slots: readonly NhStatsSkillSlotLayout[];
  readonly totalLevel: NhStatsTotalLevelLayout | null;
}

export type NhFixedSideTabId =
  | "combat"
  | "stats"
  | "quests"
  | "inventory"
...
export interface NhFixedSidePanelTabContainerLayout {
  readonly groupId: number;
  readonly widgetId: number;
  readonly childId: number;
  readonly rect: NhRect;
...
export interface NhFixedSideTabLayout {
  readonly id: NhFixedSideTabId;
  readonly groupId: number;
  readonly widgetId: number;
  readonly childId: number;
  readonly rect: NhRect;
  ...
  readonly iconSpriteId: number;
  readonly iconRect: NhRect;
  readonly container: NhFixedSidePanelTabContainerLayout;
  readonly actions: readonly string[];
}

export interface NhFixedSidePanelLayout {
  readonly groupId: number;
  readonly rect: NhRect;
  readonly backgroundSpriteId: number;
  readonly defaultTabId: NhFixedSideTabId;
  readonly tabs: readonly NhFixedSideTabLayout[];
}
...
export interface NhFixedClientLayout {
  ...
  readonly fixedViewportInterfaceContainer: NhResolvedWidget | null;
  readonly viewport: NhViewport;
  readonly minimapWidget: NhResolvedWidget | null;
  readonly compassWidget: NhResolvedWidget | null;
  readonly chatbox: NhMountedInterfaceLayout | null;
  readonly sidePanel: NhFixedSidePanelLayout | null;
  readonly sidePanelInterfaces: Partial<Record<NhFixedSideTabId, NhMountedInterfaceLayout>>;
  readonly combatPanel: NhCombatPanelLayout | null;
  readonly equipmentPanel: NhEquipmentPanelLayout | null;
  readonly prayerPanel: NhPrayerPanelLayout | null;
  readonly spellbookPanel: NhSpellbookPanelLayout | null;
  readonly spellbookPanels: Partial<Record<NhSpellbookId, NhSpellbookPanelLayout>>;
  readonly statsPanel: NhStatsPanelLayout | null;
  readonly inventoryGrid: NhInventoryGridLayout | null;
  readonly orbs: readonly NhFixedOrbLayout[];
  readonly xpDropOrb: NhXpDropOrbLayout | null;
}

export interface NhFixedClientCssLayout {
  readonly scale: number;
  readonly surfaceRect: NhRect;
  readonly viewportRect: NhRect;
  readonly minimapRect: NhRect | null;
  readonly compassRect: NhRect | null;
}
...
export const NH_INVENTORY_GROUP_ID = 149;
export const NH_ORB_GROUP_ID = 160;
export const NH_CHATBOX_GROUP_ID = 162;
export const NH_COMBAT_GROUP_ID = 593;
export const NH_SKILLS_GROUP_ID = 320;
export const NH_EQUIPMENT_GROUP_ID = 387;
export const NH_PRAYER_GROUP_ID = 541;
export const NH_SPELLBOOK_GROUP_ID = 218;
export const NH_CLAN_CHAT_GROUP_ID = 7;
export const NH_NOTICEBOARD_GROUP_ID = 720;
export const NH_FRIENDS_GROUP_ID = 429;
export const NH_IGNORES_GROUP_ID = 432;
export const NH_LOGOUT_GROUP_ID = 182;
export const NH_OPTIONS_GROUP_ID = 261;
export const NH_EMOTES_GROUP_ID = 216;
export const NH_MUSIC_GROUP_ID = 239;
...
const spellbookDefaultId: NhSpellbookId = "ancient";
...
const spellbookVarbitValueById: Readonly<Record<NhSpellbookId, number>> = {
  standard: 0,
  ancient: 1,
  lunar: 2,
  arceuus: 3
...
export function resolveNhFixedClientLayout(
  ...
  return {
    ...
    equipmentPanel: resolveNhEquipmentPanel(sidePanelInterfaces.equipment ?? null),
    prayerPanel: resolveNhPrayerPanel(sidePanelInterfaces.prayer ?? null),
    spellbookPanel: spellbookPanels[spellbookDefaultId] ?? null,
    spellbookPanels,
    statsPanel: resolveNhStatsPanel(sidePanelInterfaces.stats ?? null),
    inventoryGrid: resolveNhInventoryGrid(definitions, sidePanel),
    orbs: resolveNhFixedOrbs(definitions, resolvedWidgets),
    xpDropOrb: resolveNhXpDropOrb(definitions, resolvedWidgets)
  };
}

export function scaleNhFixedClientLayout(
...
export function pointInNhRect(rect: NhRect, x: number, y: number): boolean {
  return x >= rect.x && y >= rect.y && x < rect.x + rect.width && y < rect.y + rect.height;
}

export function nhSpellTargetFlagsFromClickMask(clickMask: number): number {
  return (Math.trunc(clickMask) >> spellTargetFlagShift) & spellTargetFlagMask;
}

export function nhSelectedSpellName(spell: Pick<NhSpellbookSpellLayout, "dataText" | "isIf3" | "label" | "spellName">): string {
...
function resolveWidgetPosition(widget: NhInterfaceWidget, parentRect: NhRect, size: NhSize): { x: number; y: number } {
  return {
...
function resolveInterfaceGroupWidgets(group: NhInterfaceGroup, rootRect: NhRect): readonly NhResolvedWidget[] {
  const widgetsByParent = new Map<number, NhInterfaceWidget[]>();
  ...
  const resolvedWidgets: NhResolvedWidget[] = [];
  const resolveChildren = (parentId: number, parentRect: NhRect, parentHidden = false): void => {
...
function findWidgetByContentType(
  widgets: readonly NhResolvedWidget[],
  contentType: number
): NhResolvedWidget | null {
...
function resolveNhChatbox(
  ...
  return {
    groupId: NH_CHATBOX_GROUP_ID,
    rect: chatboxMount.rect,
    widgets: resolveInterfaceGroupWidgets(chatboxGroup, chatboxMount.rect)
  };
}

function resolveNhInventoryGrid(
  definitions: NhClientWidgetDefinitions,
  sidePanel: NhFixedSidePanelLayout | null
): NhInventoryGridLayout | null {
  const inventoryGroup = definitions.groups.find((group) => group.groupId === NH_INVENTORY_GROUP_ID);
...
function resolveNhSidePanelInterfaces(
  definitions: NhClientWidgetDefinitions,
  sidePanel: NhFixedSidePanelLayout | null
): Partial<Record<NhFixedSideTabId, NhMountedInterfaceLayout>> {
  if (!sidePanel) {
    return {};
  }

  const mountedGroups: Partial<Record<NhFixedSideTabId, number>> = {
    combat: NH_COMBAT_GROUP_ID,
    stats: NH_SKILLS_GROUP_ID,
    quests: NH_NOTICEBOARD_GROUP_ID,
    equipment: NH_EQUIPMENT_GROUP_ID,
    prayer: NH_PRAYER_GROUP_ID,
    magic: NH_SPELLBOOK_GROUP_ID,
    "clan-chat": NH_CLAN_CHAT_GROUP_ID,
    ignores: NH_IGNORES_GROUP_ID,
    friends: NH_FRIENDS_GROUP_ID,
    logout: NH_LOGOUT_GROUP_ID,
    options: NH_OPTIONS_GROUP_ID,
    emotes: NH_EMOTES_GROUP_ID,
    music: NH_MUSIC_GROUP_ID
  };
  const layouts: Partial<Record<NhFixedSideTabId, NhMountedInterfaceLayout>> = {};
...
function resolveNhCombatPanel(combatLayout: NhMountedInterfaceLayout | null): NhCombatPanelLayout | null {
  if (!combatLayout) {
    return null;
  }

  const findCombatWidget = (childId: number): NhResolvedWidget | undefined =>
    combatLayout.widgets.find((entry) => entry.widget.groupId === NH_COMBAT_GROUP_ID && entry.widget.childId === childId);
  ...
  const autocastControls: readonly (NhCombatAutocastControlLayout | null)[] = [
  ...
  return {
    ...
    combatLevel,
    styleSlots,
    autocastControls: autocastControls.filter((control): control is NhCombatAutocastControlLayout => control !== null),
...
function combatAutocastActionRect(actionRect: NhRect, fallbackRect: NhRect): NhRect {
...
function positiveSpriteId(widget: NhResolvedWidget | undefined): number | null {
...
function combatTextLayout(widget: NhResolvedWidget | undefined): NhCombatTextLayout | null {
...
function resolveNhEquipmentPanel(
  equipmentLayout: NhMountedInterfaceLayout | null
): NhEquipmentPanelLayout | null {
  if (!equipmentLayout) {
    return null;
  }
  ...
  const slots = fixedEquipmentSlotSpecs.flatMap((spec) => {
    const widget = equipmentLayout.widgets.find(
      (entry) => entry.widget.groupId === NH_EQUIPMENT_GROUP_ID && entry.widget.childId === spec.childId
    );
    if (!widget) {
      return [];
    }
  ...
  const utilityButtons = fixedEquipmentUtilityButtonSpecs.flatMap((spec) => {
    ...
    const sprite = equipmentLayout.widgets.find(
      (entry) => entry.widget.groupId === NH_EQUIPMENT_GROUP_ID && entry.widget.childId === spec.spriteChildId
    );
...
function resolveNhStatsPanel(statsLayout: NhMountedInterfaceLayout | null): NhStatsPanelLayout | null {
  if (!statsLayout) {
    return null;
  }
  ...
  const slots = fixedStatsSlotSpecs.flatMap((spec, sourceOrder) => {
    const widget = statsLayout.widgets.find(
      (entry) => entry.widget.groupId === NH_SKILLS_GROUP_ID && entry.widget.childId === spec.childId
    );
    if (!widget) {
      return [];
    }
  ...
  const totalContainer = statsLayout.widgets.find(
    (entry) => entry.widget.groupId === NH_SKILLS_GROUP_ID && entry.widget.childId === 24
  );
  const leftSprite = statsLayout.widgets.find(
    (entry) => entry.widget.groupId === NH_SKILLS_GROUP_ID && entry.widget.childId === 25
  );
  const rightSprite = statsLayout.widgets.find(
    (entry) => entry.widget.groupId === NH_SKILLS_GROUP_ID && entry.widget.childId === 26
  );
  const textWidget = statsLayout.widgets.find(
    (entry) => entry.widget.groupId === NH_SKILLS_GROUP_ID && entry.widget.childId === 27
  );
  const totalLevel =
...
function resolveNhPrayerPanel(prayerLayout: NhMountedInterfaceLayout | null): NhPrayerPanelLayout | null {
  if (!prayerLayout) {
    return null;
  }

  const container = prayerLayout.widgets.find(
    (entry) => entry.widget.groupId === NH_PRAYER_GROUP_ID && entry.widget.childId === prayerContainerChildId
  );
  if (!container) {
    return null;
  }
  ...
  const slots = fixedPrayerSlotSpecs.flatMap((spec, sourceOrder) => {
    const widget = prayerLayout.widgets.find(
      (entry) => entry.widget.groupId === NH_PRAYER_GROUP_ID && entry.widget.childId === spec.childId
    );
    if (!widget) {
      return [];
    }
...
function resolveNhSpellbookPanels(
  spellbookLayout: NhMountedInterfaceLayout | null,
  definitions: NhSpellbookDefinitions | null
): Partial<Record<NhSpellbookId, NhSpellbookPanelLayout>> {
  const panels: Partial<Record<NhSpellbookId, NhSpellbookPanelLayout>> = {};
  if (!spellbookLayout || !definitions) {
    return panels;
  }

  for (const spellbook of definitions.spellbooks) {
    if (spellbook.groupId !== NH_SPELLBOOK_GROUP_ID) {
      continue;
    }
    const panel = resolveNhSpellbookPanel(spellbookLayout, spellbook);
    if (panel) {
...
function resolveNhSpellbookPanel(
  spellbookLayout: NhMountedInterfaceLayout,
  spellbook: NhSpellbookDefinition
): NhSpellbookPanelLayout | null {
  if (spellbook.spells.length === 0) {
    return null;
  }

  const parent = spellbookLayout.widgets.find(
    (entry) => entry.widget.groupId === NH_SPELLBOOK_GROUP_ID && entry.widget.childId === spellbookParentChildId
  );
  const bounds = spellbookLayout.widgets.find(
    (entry) => entry.widget.groupId === NH_SPELLBOOK_GROUP_ID && entry.widget.childId === spellbookBoundsChildId
  );
  if (!parent || !bounds) {
    return null;
  }
  ...
  const spells = spellbook.spells.flatMap((spell) => {
    ...
    return [
      {
        ...spellLayout,
        selectedSpellName: nhSelectedSpellName(spellLayout)
      }
    ];
  });
...
function resolveNhFixedOrbs(
  definitions: NhClientWidgetDefinitions,
  fixedWidgets: readonly NhResolvedWidget[]
): readonly NhFixedOrbLayout[] {
  const orbGroup = definitions.groups.find((group) => group.groupId === NH_ORB_GROUP_ID);
  const minimapWidget = findWidgetByContentType(fixedWidgets, NH_MINIMAP_CONTENT_TYPE);
...
function resolveNhXpDropOrb(
  definitions: NhClientWidgetDefinitions,
  fixedWidgets: readonly NhResolvedWidget[]
): NhXpDropOrbLayout | null {
  const orbGroup = definitions.groups.find((group) => group.groupId === NH_ORB_GROUP_ID);
  const minimapWidget = findWidgetByContentType(fixedWidgets, NH_MINIMAP_CONTENT_TYPE);
  const minimapRoot = minimapWidget
    ? fixedWidgets.find((entry) => entry.widget.id === minimapWidget.widget.parentId)
  ...
  const widget = orbWidgets.find(
    (entry) => entry.widget.groupId === NH_ORB_GROUP_ID && entry.widget.childId === 1 && entry.widget.spriteId === 1196
  );
  if (!widget) {
    return null;
  }
...
function findResolvedDescendantSprite(
  widgets: readonly NhResolvedWidget[],
  widgetsById: ReadonlyMap<number, NhInterfaceWidget>,
  spriteId: number,
...
function findResolvedDescendant(
  ...
  widgetsById: ReadonlyMap<number, NhInterfaceWidget>,
  ancestorId: number,
  predicate: (widget: NhInterfaceWidget) => boolean
): NhResolvedWidget | undefined {
  return widgets.find(
    (entry) =>
...
function relativeWidgetRect(widget: NhResolvedWidget, parent: NhResolvedWidget): NhRect {
  return {
...
function scaleRect(rect: NhRect, surfaceRect: NhRect, scale: number): NhRect {
  return {
