export const KRONOS_CONTEXT_MENU_FONT_KEY = "bold12";

export interface KronosClientFontDefinition {
  readonly key: string;
...
export type KronosClientFontStore = ReadonlyMap<string, KronosClientFontDefinition>;

export interface KronosClientFontGlyphLayout {
...
export function createKronosClientFontStore(source: unknown): KronosClientFontStore {
  ...
  const store = new Map<string, KronosClientFontDefinition>();
...
export function kronosClientFontDefinition(
  fonts: KronosClientFontStore,
  key: string
): KronosClientFontDefinition | null {
  return fonts.get(key) ?? null;
}

export function kronosClientFontDefinitionById(
  fonts: KronosClientFontStore,
...
export function kronosClientFontStringWidth(font: KronosClientFontDefinition, text: string | null | undefined): number {
  if (!text) {
    return 0;
  }
...
export function layoutKronosClientFontGlyphs(
  font: KronosClientFontDefinition,
  text: string | null | undefined
): readonly KronosClientFontGlyphLayout[] {
  if (!text) {
    return [];
  }

  const glyphs: KronosClientFontGlyphLayout[] = [];
  let tagStart = -1;
  let previous = -1;
  let x = 0;
...
function normalizeClientFontDefinition(source: unknown, fallbackKey: string): KronosClientFontDefinition | null {
  if (!source || typeof source !== "object") {
    return null;
  }
