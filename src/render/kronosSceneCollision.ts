...
export interface KronosArenaMetadata {
  readonly bounds: {
  ...
  readonly tiles: readonly KronosArenaTile[];
}

export interface KronosArenaTile {
  readonly x: number;
  readonly y: number;
  readonly plane: number;
...
export interface KronosArenaObjectPlacement {
  readonly id: number;
...
export interface KronosWorldTile {
  readonly x: number;
  readonly y: number;
  readonly plane: number;
}

export interface KronosSceneCollision {
  ...
  sceneToWorldTile(tile: RuntimeTile): KronosWorldTile;
  worldToSceneTile(tile: KronosWorldTile): RuntimeTile;
  getFlagWorld(x: number, y: number): number;
  getProjectileFlagWorld(x: number, y: number): number;
  sampleHeight(tile: RuntimeTile): number;
}

export interface KronosSceneOffset {
  readonly x: number;
  readonly y: number;
  readonly z: number;
}

export interface KronosScenePoint {
  readonly x: number;
  readonly y: number;
  readonly z: number;
}

export interface KronosArenaTileSceneCorners {
  readonly southWest: KronosScenePoint;
  readonly southEast: KronosScenePoint;
  readonly northEast: KronosScenePoint;
  readonly northWest: KronosScenePoint;
}
...
export const KRONOS_OBJECT_MASK = 0x100;
export const KRONOS_UNMOVABLE_MASK = 0x200000;
export const KRONOS_FLOOR_DECORATION_MASK = 0x40000;
export const KRONOS_PROJECTILE_MASK = 0x20000;
export const KRONOS_BLOCK_MOVEMENT_FULL =
  KRONOS_OBJECT_MASK | KRONOS_FLOOR_DECORATION_MASK | KRONOS_UNMOVABLE_MASK;

export const KRONOS_WEST_MASK = 0x1240108;
export const KRONOS_EAST_MASK = 0x1240180;
export const KRONOS_SOUTH_MASK = 0x1240102;
export const KRONOS_NORTH_MASK = 0x1240120;
export const KRONOS_SOUTH_WEST_MASK = 0x124010e;
export const KRONOS_NORTH_WEST_MASK = 0x1240138;
export const KRONOS_SOUTH_EAST_MASK = 0x1240183;
export const KRONOS_NORTH_EAST_MASK = 0x12401e0;

export function snapKronosTile(tile: RuntimeTile): RuntimeTile {
  return {
    x: snapAxis(tile.x),
    z: snapAxis(tile.z)
  };
...
export function kronosArenaWorldTileCenterToScene(
  arena: KronosArenaMetadata,
  sceneOffset: KronosSceneOffset,
  worldX: number,
  worldY: number
): RuntimeTile {
...
export function kronosArenaTileSceneCorners(
  arena: KronosArenaMetadata,
  sceneOffset: KronosSceneOffset,
...
export function buildKronosSceneCollision(
  arena: KronosArenaMetadata,
  objects: readonly KronosArenaObjectPlacement[],
  sceneOffset: KronosSceneOffset
): KronosSceneCollision {
  ...
  const sceneToWorldTile = (tile: RuntimeTile): KronosWorldTile =>
    sceneTileToWorldTile(arena, sceneOffset, tile);

  const worldToSceneTile = (tile: KronosWorldTile): RuntimeTile =>
  ...
  for (const key of walkableWorldTiles) {
    ...
    if ((getFlagWorld(x, y) & KRONOS_BLOCK_MOVEMENT_FULL) !== 0) {
  ...
  const canStand = (tile: RuntimeTile): boolean => {
    ...
    return (
      ...
      (getFlagWorld(world.x, world.y) & KRONOS_BLOCK_MOVEMENT_FULL) === 0
    );
  };
...
export function clientObjectFootprint(placement: KronosArenaObjectPlacement): { readonly sizeX: number; readonly sizeY: number } {
...
function addObjectPlacementCollision(
  ...
  if (type === 22) {
    if (interactType === 1) {
      addFlagWorld(placement.x, placement.y, KRONOS_FLOOR_DECORATION_MASK);
    }
    return;
  }
...
function addRectangleCollision(
  ...
  const mask = KRONOS_OBJECT_MASK | (solid ? KRONOS_PROJECTILE_MASK : 0);
...
function allowEntranceWorld(
  ...
  if (deltaX === -1 && deltaY === 0) {
    return (getFlagWorld(x - 1, y) & KRONOS_WEST_MASK) === 0;
  }
  if (deltaX === 1 && deltaY === 0) {
    return (getFlagWorld(x + 1, y) & KRONOS_EAST_MASK) === 0;
  }
  if (deltaX === 0 && deltaY === -1) {
    return (getFlagWorld(x, y - 1) & KRONOS_SOUTH_MASK) === 0;
  }
  if (deltaX === 0 && deltaY === 1) {
    return (getFlagWorld(x, y + 1) & KRONOS_NORTH_MASK) === 0;
  }
  if (deltaX === -1 && deltaY === -1) {
    return (
      (getFlagWorld(x - 1, y - 1) & KRONOS_SOUTH_WEST_MASK) === 0 &&
      (getFlagWorld(x - 1, y) & KRONOS_WEST_MASK) === 0 &&
      (getFlagWorld(x, y - 1) & KRONOS_SOUTH_MASK) === 0
    );
  }
  if (deltaX === 1 && deltaY === -1) {
    return (
      (getFlagWorld(x + 1, y - 1) & KRONOS_SOUTH_EAST_MASK) === 0 &&
      (getFlagWorld(x + 1, y) & KRONOS_EAST_MASK) === 0 &&
      (getFlagWorld(x, y - 1) & KRONOS_SOUTH_MASK) === 0
    );
  }
  if (deltaX === -1 && deltaY === 1) {
    return (
      (getFlagWorld(x - 1, y + 1) & KRONOS_NORTH_WEST_MASK) === 0 &&
      (getFlagWorld(x - 1, y) & KRONOS_WEST_MASK) === 0 &&
      (getFlagWorld(x, y + 1) & KRONOS_NORTH_MASK) === 0
    );
  }
  if (deltaX === 1 && deltaY === 1) {
    return (
      (getFlagWorld(x + 1, y + 1) & KRONOS_NORTH_EAST_MASK) === 0 &&
      (getFlagWorld(x + 1, y) & KRONOS_EAST_MASK) === 0 &&
      (getFlagWorld(x, y + 1) & KRONOS_NORTH_MASK) === 0
    );
  }
...
function tileCornerHeights(tile: KronosArenaTile): TileCornerHeights {
