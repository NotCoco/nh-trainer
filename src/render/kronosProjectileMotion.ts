...
import { kronosClientUnitsToWorldUnits } from "./kronosOverlayProjection";

export interface KronosProjectileDefinition {
  readonly id: string;
...
export interface KronosProjectileDefinitionStore {
  readonly projectiles: readonly KronosProjectileDefinition[];
}

export type KronosProjectileDefinitionMap = ReadonlyMap<string, KronosProjectileDefinition>;

export interface KronosProjectileMotionSample {
  readonly x: number;
  readonly y: number;
  readonly z: number;
  readonly yaw: number;
...
export function kronosProjectileDefinitionForGfx(
  definitions: KronosProjectileDefinitionMap | undefined,
  projectileGfxId: number
): KronosProjectileDefinition | null {
  if (!definitions) {
    return null;
  }
...
export function kronosProjectileDuration(
  event: RuntimeRenderEvent,
  definition: KronosProjectileDefinition
): number {
...
export function kronosProjectileLifecycleFromDefinition(
  event: RuntimeRenderEvent,
  definition: KronosProjectileDefinition
  ...
  const duration = kronosProjectileDuration(event, definition);
...
export function sampleKronosProjectileMotion(
  event: RuntimeRenderEvent,
  cycle: number,
  definition: KronosProjectileDefinition
): KronosProjectileMotionSample | null {
  const lifecycle = event.projectile ?? kronosProjectileLifecycleFromDefinition(event, definition);
  if (!lifecycle) {
    return null;
  }

  return sampleKronosProjectileLifecycle(
    lifecycle,
    kronosRenderCycleToProjectileClientCycle(event, cycle, lifecycle)
  );
}

export function sampleKronosProjectileLifecycle(
  lifecycle: RuntimeProjectileLifecycle,
  clientCycle: number
): KronosProjectileMotionSample {
  ...
  const sourceOffset = kronosClientUnitsToWorldUnits(lifecycle.startDistanceOffset);
...
export function kronosRenderCycleToProjectileClientCycle(
  event: RuntimeRenderEvent,
  cycle: number,
  lifecycle: RuntimeProjectileLifecycle
): number {
...
function projectileHeightToWorldHeight(height: number): number {
  return kronosClientUnitsToWorldUnits(height * SERVER_PROJECTILE_HEIGHT_SCALE);
}
