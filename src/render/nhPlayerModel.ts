...
import type { NhLoadoutMeshMetadata } from "./nhSequencePlayback";

type PlayerBodyColors = readonly [number, number, number, number, number];

export interface NhCacheItemDefinition {
  readonly id: number;
...
export interface NhCacheModelDefinition {
  readonly id: number;
...
interface NhBodyColorDefinitions {
...
interface NhTextureDefinitionStore {
  readonly textures: readonly NhTextureDefinition[];
}

interface NhTextureDefinition {
  readonly id: number;
...
export interface NhPlayerModelSources {
  readonly cacheItems: Record<string, NhCacheItemDefinition>;
  readonly kits: Record<string, NhKitDefinition>;
  readonly cacheModels: Record<string, NhCacheModelDefinition>;
  readonly serverItems: readonly NhServerItemDefinition[];
  readonly bodyColors: NhBodyColorDefinitions;
  readonly textures?: NhTextureDefinitionStore;
}

export interface NhPlayerAppearanceModelInput {
...
export interface NhComposedPlayerModel {
...
export interface NhGroundItemModelInput {
  readonly itemId: number;
  readonly quantity: number;
}

export interface NhGroundItemModelSource {
  readonly itemId: number;
  readonly itemName: string;
  readonly quantity: number;
  readonly modelId: number;
  readonly item: NhCacheItemDefinition;
  readonly model: NhCacheModelDefinition;
}

export interface NhComposedGroundItemModel {
  readonly scene: Object3D;
  readonly source: NhGroundItemModelSource;
  readonly metadata: {
    readonly source: string;
    readonly itemId: number;
    readonly itemName: string;
...
interface NhPreparedPlayerModelPart extends NhPlayerModelPart {
  readonly sourceVertexBase: number;
  readonly sourceFaceBase: number;
}

interface NhPreparedPlayerFace {
  readonly part: NhPreparedPlayerModelPart;
  readonly faceIndex: number;
  readonly priority: number;
  readonly sourceOrder: number;
}

interface NhPainterTriangle {
...
interface NhPlayerPainterGeometryUserData {
  nhPlayerPainter?: true;
  nhTrianglePriorities?: readonly number[];
  nhTriangleMaterialIndices?: readonly number[];
  nhTriangleSourceIndices?: readonly number[];
  nhTriangleCount?: number;
  nhUsesFaceRenderPriorities?: boolean;
  nhSource?: string;
}

interface NhPlayerTextureMaterialKey {
  readonly textureId: number;
  readonly transparent: boolean;
}

interface NhClientModelLightingConfig {
  readonly ambient: number;
  readonly contrast: number;
...
const NH_UNTEXTURED_FACE_ID = -1;
const NH_TEXTURE_SIZE = 128;
const sharedNhPlayerTextureLoader = new TextureLoader();
const sharedNhPlayerTextures = new Map<string, Texture>();
const nhPainterPoint = new Vector3();
...
export function composeNhPlayerModel(
  sources: NhPlayerModelSources,
  input: NhPlayerAppearanceModelInput
): NhComposedPlayerModel {
  const serverItemById = new Map(sources.serverItems.map((item) => [item.id, item]));
  const equipmentSlots = nhEquipmentSlotsWithSequenceOverrides(
    input.equipmentSlots
      ? normalizeNhEquipmentSlots(input.equipmentSlots)
      : nhEquipmentSlotsFromLoadoutItems(input.itemIds, sources.kits, serverItemById),
    input
  );
  const mesh = createMesh();
  const parts: NhPlayerModelPart[] = [];
  ...
  if (mesh.indices.length === 0) {
    throw new Error("Nh player appearance produced no geometry");
  }
  ...
  return {
    scene,
    metadata: {
      source: "Nh PlayerAppearance.method4156 equipment slot order and cache ModelDefinition parts",
...
export function composeNhGroundItemModel(
  sources: NhPlayerModelSources,
  input: NhGroundItemModelInput
): NhComposedGroundItemModel | null {
  const source = resolveNhGroundItemModelSource(sources, input);
  if (!source) {
    return null;
  }

  const mesh = createMesh();
  const part: NhPlayerModelPart = {
    model: nhGroundItemCacheModelForSource(source),
  ...
  appendPreparedModelFaces(
    ...
    new Map(),
    nhGroundItemLightingConfig(source.item)
  );
  mesh.sourceModels.push(part.sourceModel);
  ...
  const textureDefinitions = new Map((sources.textures?.textures ?? []).map((texture) => [texture.id, texture]));
  const scene = createPlayerAppearanceScene(geometry, mesh, textureDefinitions);
  scene.userData.nhGroundItem = true;
  scene.userData.nhSource =
  ...
  return {
    ...
    metadata: {
      source:
        "Nh TileItem.vmethod3072 uses ItemDefinition.getModel(quantity); ground item model is the item inventoryModel with item resize/recolor and ambient+64 contrast+768 lighting",
      itemId: source.itemId,
      itemName: source.itemName,
      quantity: source.quantity,
      modelId: source.modelId,
...
export function resolveNhGroundItemModelSource(
  sources: NhPlayerModelSources,
  input: NhGroundItemModelInput
): NhGroundItemModelSource | null {
  ...
  const resolvedItemId = nhGroundItemQuantityVariantId(sourceItem, input.quantity);
...
export function nhGroundItemCacheModelForSource(
  source: NhGroundItemModelSource
): NhCacheModelDefinition {
...
export function normalizeNhEquipmentSlots(equipmentSlots: readonly number[]): readonly number[] {
  if (equipmentSlots.length !== 12) {
    throw new Error(`Nh player appearance requires 12 equipment slots, got ${equipmentSlots.length}`);
  }

  return equipmentSlots.map((slot, index) => {
    if (!Number.isInteger(slot) || slot < 0 || slot > 65535) {
      throw new Error(`invalid Nh equipment slot ${index}: ${slot}`);
    }
    return slot;
  });
}

export function nhEquipmentSlotsFromLoadoutItems(
  itemIds: readonly number[],
  kits: Record<string, NhKitDefinition>,
  serverItemById: ReadonlyMap<number, NhServerItemDefinition>
): readonly number[] {
  ...
  for (const itemId of itemIds) {
    ...
    equipmentSlots[(serverItem as NhEquippedServerItemDefinition).equipSlot] = itemId + 512;
  }
...
function nhEquipmentSlotsWithSequenceOverrides(
  equipmentSlots: readonly number[],
  input: Pick<NhPlayerAppearanceModelInput, "shieldOverrideId" | "weaponOverrideId">
): readonly number[] {
...
function createMesh(): NhMutableMesh {
  return {
    positions: [],
    colors: [],
    uvs: [],
...
function createPlayerAppearanceSubmesh(
  ...
  scene.name = name;
  scene.userData.nhPlayerPainter = true;
  scene.userData.nhSource =
    "Nh Model.method2375 projected depth bucket ordering and face priority pass";
  const geometryUserData = geometry.userData as NhPlayerPainterGeometryUserData;
  geometryUserData.nhPlayerPainter = true;
  geometryUserData.nhTrianglePriorities = [...mesh.trianglePriorities];
  geometryUserData.nhTriangleMaterialIndices = triangleMaterialIndices;
  geometryUserData.nhTriangleSourceIndices = [...sourceTriangles];
  geometryUserData.nhTriangleCount = sourceTriangles.length;
  geometryUserData.nhUsesFaceRenderPriorities = mesh.usesFaceRenderPriorities;
  geometryUserData.nhSource =
    "Nh Model.method2375 projected depth bucket ordering and face priority pass";
  return scene;
}
...
function playerTriangleMaterialKey(
  ...
  triangle: number,
  textureDefinitions: ReadonlyMap<number, NhTextureDefinition>
): NhPlayerTextureMaterialKey {
  const sourceFace = mesh.expandedToSourceFace[triangle * 3] ?? -1;
  const textureId = mesh.triangleTextureIds[triangle] ?? NH_UNTEXTURED_FACE_ID;
...
function playerTriangleMaterialKeyId(key: NhPlayerTextureMaterialKey): string {
  return `${key.textureId}:${key.transparent ? 1 : 0}`;
}

function createPlayerAppearanceMaterial(
  key: NhPlayerTextureMaterialKey,
  textureDefinitions: ReadonlyMap<number, NhTextureDefinition>
): MeshBasicMaterial {
  ...
  material.userData.nhSource =
    key.textureId === NH_UNTEXTURED_FACE_ID
      ...
      : "Nh Model.faceTextures textured draw path";
  return material;
}

function defaultMaleKits(kits: Record<string, NhKitDefinition>) {
...
function preparePlayerModelParts(
  mesh: NhMutableMesh,
  parts: readonly NhPlayerModelPart[]
): readonly NhPreparedPlayerModelPart[] {
...
function appendPreparedPlayerFaces(
  mesh: NhMutableMesh,
  parts: readonly NhPreparedPlayerModelPart[],
  bodyColorDefinitions: NhBodyColorDefinitions,
...
function appendPreparedModelFaces(
  mesh: NhMutableMesh,
  parts: readonly NhPreparedPlayerModelPart[],
  faceColor: (face: NhPreparedPlayerFace) => number,
  sharedVertexNormals: ReadonlyMap<string, ClientModelNormal>,
  lightingConfig: NhClientModelLightingConfig
): void {
  const litFaceCache = new Map<NhPreparedPlayerModelPart, readonly (readonly [number, number, number] | null)[]>();
  const faces: NhPreparedPlayerFace[] = [];
  let sourceOrder = 0;
  mesh.usesFaceRenderPriorities = nhComposedModelUsesFaceRenderPriorities(parts);
  ...
  for (const part of parts) {
    ...
    litFaceCache.set(
      ...
      createClientPlayerFaceColors(
        model,
        faceCount,
        (faceIndex) => faceColor({ part, faceIndex, priority: nhModelFacePriority(model, faceIndex), sourceOrder: 0 }),
        sharedVertexNormals,
        lightingConfig
      )
    );
    ...
    for (let faceIndex = 0; faceIndex < faceCount; faceIndex += 1) {
      faces.push({
        part,
        faceIndex,
        priority: nhModelFacePriority(model, faceIndex),
        sourceOrder
      });
      sourceOrder += 1;
    }
  ...
  for (const face of faces) {
    ...
    const alpha = model.faceAlphas?.[faceIndex] ?? 0;
    const textureId = model.faceTextures?.[faceIndex] ?? NH_UNTEXTURED_FACE_ID;
    const colors =
      textureId === NH_UNTEXTURED_FACE_ID
        ? litFace.map((color) => rs2hslToRgba(color, alpha))
        : litFace.map((lightness) => textureLightnessToRgba(lightness, alpha));
    const uvs = nhModelFaceTextureUvs(model, faceIndex, textureId);
    ...
    // The runtime camera mirrors X in projection, so the x,-y,z model basis keeps Nh
...
function nhComposedModelUsesFaceRenderPriorities(parts: readonly NhPreparedPlayerModelPart[]): boolean {
...
function nhModelFaceTextureUvs(
  model: NhCacheModelDefinition,
  faceIndex: number,
  textureId: number
): readonly [number, number][] {
  if (textureId === NH_UNTEXTURED_FACE_ID) {
    return [
      [0, 0],
      [0, 0],
      [0, 0]
...
function nhModelFacePriority(model: NhCacheModelDefinition, faceIndex: number): number {
...
function nhPlayerFaceDrawOrder(faces: readonly NhPreparedPlayerFace[]): readonly NhPreparedPlayerFace[] {
  const priorityBuckets = Array.from({ length: 12 }, () => [] as NhPreparedPlayerFace[]);
  const deferredFaces: NhPreparedPlayerFace[] = [];
  for (const face of faces) {
...
function createClientPlayerFaceColors(
  ...
  return Array.from({ length: faceCount }, (_, faceIndex) => {
    ...
    const texture = model.faceTextures?.[faceIndex] ?? NH_UNTEXTURED_FACE_ID;
...
function createClientPlayerSharedVertexNormals(parts: readonly NhPlayerModelPart[]): ReadonlyMap<string, ClientModelNormal> {
...
function clientVertexCoordinateKey(model: NhCacheModelDefinition, vertexIndex: number): string {
...
function clientModelRenderType(model: NhCacheModelDefinition, faceIndex: number): number {
...
function nhGroundItemLightingConfig(item: NhCacheItemDefinition): NhClientModelLightingConfig {
...
function itemModelIds(item: NhCacheItemDefinition): number[] {
...
function nhGroundItemQuantityVariantId(item: NhCacheItemDefinition, quantity: number): number {
...
function kitModelIds(kit: NhKitDefinition): number[] {
  return (kit.modelIds ?? []).filter((modelId): modelId is number => Number.isInteger(modelId) && modelId >= 0);
}

function modelVertexGroups(model: NhCacheModelDefinition): number[] {
  const vertexCount = model.vertexCount ?? model.vertexPositionsX.length;
...
function applyBodyRecolors(
  color: number,
  definitions: NhBodyColorDefinitions,
  bodyColors: PlayerBodyColors
): number {
  let output = toSignedShort(color);
...
function nhPlayerTexture(textureId: number, definition: NhTextureDefinition | undefined): Texture {
  const url = definition?.image ?? `render/textures/texture_${textureId}.png`;
  const key = `${textureId}:${url}`;
  const existing = sharedNhPlayerTextures.get(key);
  if (existing) {
    return existing;
  }

  const texture =
    typeof Image === "undefined" ? new Texture() : sharedNhPlayerTextureLoader.load(url);
  texture.name = `nh-player-texture-${textureId}`;
  texture.wrapS = RepeatWrapping;
  texture.wrapT = RepeatWrapping;
  texture.magFilter = NearestFilter;
  texture.minFilter = NearestFilter;
  texture.userData.nhTextureId = textureId;
  texture.userData.nhTextureUrl = url;
  texture.userData.nhTextureWidth = definition?.width ?? NH_TEXTURE_SIZE;
  texture.userData.nhTextureHeight = definition?.height ?? NH_TEXTURE_SIZE;
  texture.userData.nhAnimationDirection = definition?.animationDirection ?? 0;
  texture.userData.nhAnimationSpeed = definition?.animationSpeed ?? 0;
  texture.userData.nhSource =
    "Nh Texture.copy$animate / RuneLite GPU TextureManager.animate direction-speed UV offset";
  sharedNhPlayerTextures.set(key, texture);
  return texture;
}

export function updateNhAnimatedTextures(root: Object3D, clientCycle: number): void {
  ...
  root.traverse((node) => {
    ...
    for (const material of materials) {
      ...
      updateNhAnimatedTexture(texture, cycle);
      updatedTextures.add(texture);
    }
  });
}

export function updateNhPlayerModelPainterOrder(root: Object3D, camera: Camera): void {
  camera.updateMatrixWorld(true);
  camera.matrixWorldInverse.copy(camera.matrixWorld).invert();
  root.updateMatrixWorld(true);
  root.traverse((node) => {
    const mesh = node as Mesh<BufferGeometry, MeshBasicMaterial | MeshBasicMaterial[]>;
    if (!mesh.isMesh || !mesh.geometry.userData.nhPlayerPainter) {
      return;
    }
    updateNhPlayerMeshPainterOrder(mesh, camera);
  });
}

function updateNhPlayerMeshPainterOrder(
  mesh: Mesh<BufferGeometry, MeshBasicMaterial | MeshBasicMaterial[]>,
  camera: Camera
): void {
  const geometry = mesh.geometry;
  const userData = geometry.userData as NhPlayerPainterGeometryUserData;
  const triangleCount = userData.nhTriangleCount ?? 0;
  const priorities = userData.nhTrianglePriorities;
  const materialIndices = userData.nhTriangleMaterialIndices;
  const sourceTriangles = userData.nhTriangleSourceIndices ?? Array.from({ length: triangleCount }, (_, index) => index);
  ...
  const triangles: NhPainterTriangle[] = [];
  for (const triangle of sourceTriangles) {
    triangles.push({
      triangle,
      priority: priorities[triangle] ?? 0,
      depth: nhPainterTriangleDepth(mesh, camera, position, triangle),
      sourceOrder: triangle
    });
  }

  const ordered = nhPainterTriangleOrder(triangles, userData.nhUsesFaceRenderPriorities === true);
...
function nhPainterTriangleDepth(
  mesh: Mesh<BufferGeometry, MeshBasicMaterial | MeshBasicMaterial[]>,
  camera: Camera,
  ...
  for (let vertexOffset = 0; vertexOffset < 3; vertexOffset += 1) {
    nhPainterPoint.fromBufferAttribute(position, triangle * 3 + vertexOffset);
    nhPainterPoint.applyMatrix4(mesh.matrixWorld);
    nhPainterPoint.applyMatrix4(camera.matrixWorldInverse);
    depth += -nhPainterPoint.z;
  }
  return depth / 3;
}

function nhPainterTriangleOrder(
  triangles: readonly NhPainterTriangle[],
  usesFaceRenderPriorities: boolean
): readonly NhPainterTriangle[] {
  ...
  const priorityBuckets = Array.from({ length: 12 }, () => [] as NhPainterTriangle[]);
  ...
  const depth12 = nhPriorityAverageDepth(priorityBuckets, priorityDepthSums, 1, 2);
  const depth34 = nhPriorityAverageDepth(priorityBuckets, priorityDepthSums, 3, 4);
  const depth68 = nhPriorityAverageDepth(priorityBuckets, priorityDepthSums, 6, 8);
  const lateFaces = [...priorityBuckets[10], ...priorityBuckets[11]];
  let lateIndex = 0;
  const output: NhPainterTriangle[] = [];
...
function nhPriorityAverageDepth(
  buckets: readonly (readonly NhPainterTriangle[])[],
...
function updateNhAnimatedTexture(texture: Texture, clientCycle: number): void {
  const direction = texture.userData.nhAnimationDirection as number | undefined;
  const speed = texture.userData.nhAnimationSpeed as number | undefined;
  if (!direction || !speed) {
    return;
  }

  const width = Math.max(1, Number(texture.userData.nhTextureWidth) || NH_TEXTURE_SIZE);
  const height = Math.max(1, Number(texture.userData.nhTextureHeight) || NH_TEXTURE_SIZE);
