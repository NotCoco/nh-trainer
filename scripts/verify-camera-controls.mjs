...
import { kronosClientCameraPresets } from "./render-reference-targets.mjs";

const projectRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const sourcePath = path.join(projectRoot, "src", "render", "kronosClientCamera.ts");
...
if (
  ...
  runtimeViewerSource.includes("boundary.cameraRig.clientAngles.yaw, KRONOS_CAMERA_MAX_PITCH") ||
  runtimeViewerSource.includes("KRONOS_CAMERA_FALLBACK_FOV_DEGREES") ||
  ...
  throw new Error("RuntimeSceneViewer should use source-backed Kronos client camera constants instead of handmade orbit/FOV fallbacks.");
}
...
const {
  KRONOS_CAMERA_MIN_PITCH,
  KRONOS_CAMERA_MAX_PITCH,
  KRONOS_CAMERA_DEFAULT_PITCH_UNITS,
  KRONOS_CAMERA_DEFAULT_YAW_UNITS,
  ...
  KRONOS_CAMERA_INNER_ZOOM_LIMIT,
  KRONOS_CAMERA_SCROLL_WHEEL_INCREMENT,
  KRONOS_CLIENT_TO_SCENE_UNITS,
  KRONOS_CAMERA_DEFAULT_FOLLOW_HEIGHT_UNITS,
  KRONOS_CAMERA_FOCUS_RESET_DISTANCE_SCENE_UNITS,
  KRONOS_RUNTIME_CAMERA_PRESETS,
  kronosClientCameraOffset,
  kronosClientSceneCameraOffset,
  kronosClientCameraBaseDistance,
  kronosCameraFollowHeightUnits,
...
function runSequence(initial, sequence) {
  return sequence.reduce(
    (state, keys) => ({
      actual: updateKronosCameraAngles(state.actual, keys),
...
for (const position of [
  ...
  actualMouse = updateKronosCameraAnglesFromMouseDrag(actualMouse.angles, actualMouse.mouse, position);
...
if (pitchUpperClamp.actual.pitch !== KRONOS_CAMERA_MAX_PITCH) {
  throw new Error(`expected upper pitch clamp ${KRONOS_CAMERA_MAX_PITCH}, got ${pitchUpperClamp.actual.pitch}`);
}
...
if (pitchLowerClamp.actual.pitch !== KRONOS_CAMERA_MIN_PITCH) {
  throw new Error(`expected lower pitch clamp ${KRONOS_CAMERA_MIN_PITCH}, got ${pitchLowerClamp.actual.pitch}`);
}

const releaseStart = updateKronosCameraAngles({ yaw: 0, pitch: 200, camAngleDX: 0, camAngleDY: 0 }, left);
if (!isKronosCameraMoving(releaseStart)) {
  throw new Error("camera velocity should remain active immediately after a held key tick");
}
let releaseEnd = releaseStart;
for (let index = 0; index < 8; index += 1) {
  releaseEnd = updateKronosCameraAngles(releaseEnd, noKeys);
}
if (isKronosCameraMoving(releaseEnd)) {
  throw new Error(`camera velocity should decay to zero after release, got ${JSON.stringify(releaseEnd)}`);
}

const fixedCameraOffset = kronosClientCameraOffset({ yaw: 256, pitch: KRONOS_CAMERA_DEFAULT_PITCH_UNITS }, 334);
assertSame(
  "fixed viewport camera orbit offset",
  fixedCameraOffset,
  referenceCameraOffset({ yaw: 256, pitch: KRONOS_CAMERA_DEFAULT_PITCH_UNITS }, 334)
);
const tallCameraOffset = kronosClientCameraOffset({ yaw: 640, pitch: 383 }, 434);
assertSame("tall viewport camera orbit offset", tallCameraOffset, referenceCameraOffset({ yaw: 640, pitch: 383 }, 434));

const zoomedCameraOffset = kronosClientCameraOffset({ yaw: 256, pitch: KRONOS_CAMERA_DEFAULT_PITCH_UNITS }, 334, {
  zoomHeight: 128,
  zoomWidth: 128
});
assertSame(
  ...
  zoomedCameraOffset,
  referenceCameraOffset({ yaw: 256, pitch: KRONOS_CAMERA_DEFAULT_PITCH_UNITS }, 334, { zoomHeight: 128, zoomWidth: 128 })
);

const sceneOffset = kronosClientSceneCameraOffset({ yaw: 256, pitch: KRONOS_CAMERA_DEFAULT_PITCH_UNITS }, 334);
assertSame("scene camera offset scaling", sceneOffset, {
  x: fixedCameraOffset.x * KRONOS_CLIENT_TO_SCENE_UNITS,
  y: fixedCameraOffset.y * KRONOS_CLIENT_TO_SCENE_UNITS,
  z: fixedCameraOffset.z * KRONOS_CLIENT_TO_SCENE_UNITS,
  distance: fixedCameraOffset.distance * KRONOS_CLIENT_TO_SCENE_UNITS,
  zoomScale: fixedCameraOffset.zoomScale
});

const wheelZoomIn = updateKronosCameraZoomFromScrollWheel({ zoomHeight: 512, zoomWidth: 512 }, 1);
assertSame("scroll wheel zoom step", wheelZoomIn, referenceWheelZoom({ zoomHeight: 512, zoomWidth: 512 }, 1));
const wheelZoomClamp = updateKronosCameraZoomFromScrollWheel({ zoomHeight: 138, zoomWidth: 138 }, 1);
assertSame("scroll wheel zoom outer clamp", wheelZoomClamp, { zoomHeight: 128, zoomWidth: 128 });
const wheelZoomOut = updateKronosCameraZoomFromScrollWheel({ zoomHeight: 512, zoomWidth: 512 }, -1);
assertSame("scroll wheel zoom out step", wheelZoomOut, { zoomHeight: 537, zoomWidth: 537 });
const wheelZoomInnerClamp = updateKronosCameraZoomFromScrollWheel({ zoomHeight: 886, zoomWidth: 886 }, -1);
assertSame("scroll wheel zoom inner clamp", wheelZoomInnerClamp, { zoomHeight: 896, zoomWidth: 896 });
assertSame("options zoom slider midpoint", kronosCameraZoomFromSliderOffset(48, 96), referenceSliderZoom(48, 96));
if (kronosCameraZoomSliderOffset({ zoomHeight: 512, zoomWidth: 512 }, 334, 96) !== referenceSliderOffset({ zoomHeight: 512, zoomWidth: 512 }, 334, 96)) {
  throw new Error("options zoom slider offset drifted from OptionsPanelZoomUpdater.rs2asm");
}
if (kronosCameraFollowHeightUnits({ zoomHeight: 512, zoomWidth: 512 }, 334) !== referenceFollowHeight({ zoomHeight: 512, zoomWidth: 512 }, 334)) {
  throw new Error("camera follow height should be derived from ZoomHandler.rs2asm cam_setfollowheight formula");
}

for (const [name, expected] of Object.entries(kronosClientCameraPresets)) {
  const runtimePreset = KRONOS_RUNTIME_CAMERA_PRESETS[name];
  assertSame(`runtime camera preset ${name}`, runtimePreset, { yaw: expected.yaw, pitch: expected.pitch });
  assertSame(`runtime camera preset state ${name}`, kronosRuntimeCameraPreset(name), {
    yaw: expected.yaw,
    pitch: expected.pitch,
    camAngleDX: 0,
    camAngleDY: 0
  });
  const expectedZoomedDistance = Math.trunc((kronosClientCameraBaseDistance(expected.pitch) * KRONOS_CAMERA_DEFAULT_ZOOM.zoomHeight) / 256);
  ...
  if (expected.focalHeightOffset !== KRONOS_CAMERA_DEFAULT_FOLLOW_HEIGHT_UNITS) {
...
assertIncludes("runtime default camera fov", runtimeViewerSource, "new PerspectiveCamera(KRONOS_CAMERA_DEFAULT_FOV_DEGREES");
assertIncludes("runtime default camera viewport height", runtimeViewerSource, "KRONOS_CAMERA_DEFAULT_VIEWPORT_HEIGHT");
assertIncludes(
...
assertIncludes("runtime mouse camera source update", source, "updateKronosCameraAnglesFromMouseDrag");
assertIncludes("runtime wheel camera zoom source update", runtimeViewerSource, "updateKronosCameraZoomFromScrollWheel");
...
if (kronosCameraFollowHeightSceneUnits() !== KRONOS_CAMERA_DEFAULT_FOLLOW_HEIGHT_UNITS * KRONOS_CLIENT_TO_SCENE_UNITS) {
  throw new Error("camera follow height scene conversion drifted from the client default follow-height constant");
}
if (KRONOS_CAMERA_DEFAULT_VIEWPORT_HEIGHT !== 334 || KRONOS_CAMERA_DEFAULT_VIEWPORT_ZOOM !== 256 || KRONOS_CAMERA_DEFAULT_ZOOM.zoomHeight !== 512 || KRONOS_CAMERA_DEFAULT_ZOOM.zoomWidth !== 512) {
  throw new Error("default runtime camera constants should match fixed viewport projection plus camera_do_zoom fallback");
}
if (KRONOS_CAMERA_OUTER_ZOOM_LIMIT !== 128 || KRONOS_CAMERA_INNER_ZOOM_LIMIT !== 896 || KRONOS_CAMERA_SCROLL_WHEEL_INCREMENT !== 25) {
  throw new Error("camera zoom limits and wheel increment should match the source client scripts");
}
if (KRONOS_CAMERA_DEFAULT_PITCH_UNITS !== 128 || KRONOS_CAMERA_DEFAULT_YAW_UNITS !== 0) {
  throw new Error("default runtime camera angle constants should match Client.java camAngleX/camAngleY initialization");
}
if (KRONOS_RUNTIME_CAMERA_PRESETS.isometric.pitch !== KRONOS_CAMERA_DEFAULT_PITCH_UNITS) {
...
if (kronosViewportZoomToFovDegrees(KRONOS_CAMERA_DEFAULT_VIEWPORT_HEIGHT, KRONOS_CAMERA_DEFAULT_VIEWPORT_ZOOM) !== KRONOS_CAMERA_DEFAULT_FOV_DEGREES) {
  throw new Error("default runtime camera FOV should be derived from Kronos viewport zoom");
}
if (Math.abs(KRONOS_CAMERA_DEFAULT_FOV_DEGREES - 66.23633715063609) > 1e-12) {
  throw new Error(`default runtime camera FOV drifted from fixed viewport source value: ${KRONOS_CAMERA_DEFAULT_FOV_DEGREES}`);
}
if (smoothKronosCameraFocusAxis(0, KRONOS_CAMERA_FOCUS_RESET_DISTANCE_SCENE_UNITS + 0.001) !== KRONOS_CAMERA_FOCUS_RESET_DISTANCE_SCENE_UNITS + 0.001) {
  throw new Error("camera focus should snap when it exceeds the source 500-unit reset window");
}
if (smoothKronosCameraFocusAxis(0, 1.6) !== 0.1) {
  ...
  JSON.stringify(
    {
      ...
      wheelZoomClamp,
      sceneOffset,
      capturePresets: kronosClientCameraPresets
    },
    null,
    2
  )
