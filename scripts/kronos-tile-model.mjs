export const KRONOS_TILE_MODEL_VERTEX_TYPES = [
  [1, 3, 5, 7],
  [1, 3, 5, 7],
  [1, 3, 5, 7],
  [1, 3, 5, 7, 6],
...
export const KRONOS_TILE_MODEL_FACE_TYPES = [
  [0, 1, 2, 3, 0, 0, 1, 3],
  [1, 1, 2, 3, 1, 0, 1, 3],
  [0, 1, 2, 3, 1, 0, 1, 3],
...
export function createKronosTileModel(options) {
  const shape = options.shape;
  const rotation = options.rotation & 3;
  const vertexTypes = KRONOS_TILE_MODEL_VERTEX_TYPES[shape];
  const faceTypes = KRONOS_TILE_MODEL_FACE_TYPES[shape];
  if (!vertexTypes || !faceTypes) {
    throw new Error(`unsupported Kronos TileModel shape ${shape}`);
  }
